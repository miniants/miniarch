...
<script>
    ...
    app.controller('mainCtrl', function($scope, $http, Data) {
        $http.get(mvcRoot+"AdminService/adminIndex.json")
    ...
    app.service('DataService',function($http){
        ...
        ctx.fetchUris=function(pagination,callback,options){
            ctx.fetch("AdminService/uris",pagination,callback,options);
        }
        ...
        ctx.initRoles=function(callback,options){
            ctx.fetch("AdminService/roles",1,function(roles,rolesVo){
                ctx.roles=roles;
                ctx.rolesVo=rolesVo;
    ...
    app.directive('rmxGrid',function($scope,DataService){
        var beans=[1];
//        DataService.fetch("AdminService/uris",1,function(uris){
//            beans=uris;
//        });
        return {
            restrict: 'A',
...
,                name:'111'
    ...
    app.controller('rolesCtrl', function($scope, $http, Data,DataService) {
        ...
        $scope.selectRole=function(role){
            ...
            if(!role.authUris){//如果没有权限,则取后台取一下
                $http.get(mvcRoot+"AdminService/roleUris/"+(role.id)+".json").success(function (response) {
        ...
        $scope.saveRole=function(role){
            $http.post(mvcRoot+"AdminService/saveRole/"+(role.id||-1)+".json",role).success(function (response) {
        ...
        $scope.delRole=function(role){
            if(role._isNew){
                ...
                $http.post(mvcRoot+"AdminService/delRole/"+(role.id)+".json",role).success(function (response) {
                    if(!response.status){
                        alert(response.msg);
                    }else{
                        $scope.roles.splice($scope.roles.indexOf(role),1);//删除
    ...
    app.controller('usersCtrl', BeanEditWithListFieldCtrlFactory({
        beansUri:'AdminService/users',
        delBeanUri:'AdminService/delUser/',
        saveBeanUri:'AdminService/saveUser/',
        propsOfBeanUri:'AdminService/rolesOfUser/',
        propsUri:'AdminService/roles'})
    );

    app.controller('pbyCtrl',function($scope, $http, DataService){

        $scope.turnoffAll=function(){
            $http.get(mvcRoot+"AdminService/turnoffAll.json").success(function (response) {
                //role.authUris=response.datas || [];
            });
        };
        $scope.turnonAll=function(){
            $http.get(mvcRoot+"AdminService/turnonAll.json").success(function (response) {
                //role.authUris=response.datas || [];
            });
        };
    });
