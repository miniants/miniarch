...
public class RsqlMonitorAspect {
	...
	public Object doAroundRemexSqlExecute(final ProceedingJoinPoint pjp) throws Throwable {
		...
		if (isDebug) {
			...
			SqlBean<Modelable> sqlBean = dbCvo.getSqlBean();
			String sqlString = sqlBean.getPrettySqlString();
			String paramString = sqlBean.getNamedParams().toString();

			debugMsg = new StringBuilder().append("\r\n——SQL语句：\r\n").append(sqlString)
		...
		try {
			...
			if(null!=debugMsg) msg.append(debugMsg);
		} catch (Exception e) {
