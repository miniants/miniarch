...
public class CreatePluginsConfig {
	public static void main(final String[] args) {
		...
		new CreatePluginsConfig("C:\\Program Files\\Genuitec\\MyEclipse-8.6\\Common\\language\\plugins").print(); //汉化包插件路径

	}

	private String path;

	public CreatePluginsConfig(final String path) {
		this.path = path;
	}
	...
	public void print() {
		List<String> list = getFileList(this.path);
		if (list == null) {
			return;
		}
		...
		for (int i = 0; i < length; i++) {
			...
			if (file.isDirectory()) {
				...
				result = filename1 + "," + filename2 + ",file:/" + this.path + "\\"
						+ fileName + "\\,4,false";
