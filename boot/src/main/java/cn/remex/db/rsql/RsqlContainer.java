...
public class RsqlContainer implements Container, RsqlConstants {
    ...
    public <T extends Modelable> DbCvo<T> createDbCvo(Class<T> beanClass) {
        innerDbCvo = new DbCvo<>(beanClass);
    ...
    public <T extends Modelable> DbCvo<T> createDbCvo(String sqlString, Map<String, Object> params) {
        innerDbCvo = new DbCvo(obtainManualSql(sqlString), params);
        innerDbCvo.setContainer(this);
        return (DbCvo<T>) innerDbCvo;
    }
    ...
    @Override
    public DbRvo update() {
        innerDbCvo.initParam();
        return RsqlDao.getDefaultRemexDao().executeQuery(innerDbCvo);
    }
    ...
    public <T extends Modelable> DbRvo delete(final T o) {
        ...
        DbCvo<T> dbCvo;
        if (null == innerDbCvo)
            dbCvo = new DbCvo<>(clazz, SqlOper.del);
        else
            dbCvo = innerDbCvo;
    ...
    public <T extends Modelable> DbRvo store(final T obj) {
        DbCvo dbCvo;
        if (null == innerDbCvo)
            dbCvo = new DbCvo<>((Class<Modelable>) obj.getClass(), SqlOper.store, DT_whole);
        else {
    ...
    public <T extends Modelable> DbRvo updateByFields(T bean, String saveFields, String constraintFields) {
        Assert.isNull(innerDbCvo, "该方法禁止dbCvo方法，只能通过ContainerFactory.getSession()直接调用！", RsqlException.class);
        DbCvo<T> dbCvo = new DbCvo<>((Class<T>) bean.getClass());
    ...
    @Override
    public DbRvo executeQuery(final String sql, final HashMap<String, Object> params) {
    ...
    private <T> void updateOne2One(T obj, Map<OneToOne, Modelable> map) {
        map.forEach((oto, foreignBean) -> {
            if (oto != null) {
                ...
                Assert.isTrue(tgtFieldName != null, "OneToOne的mappedBy对方属性的值不能为空！");
    ...
    private Map<String, Object> maplizeObject(final Object o, SqlOper oper, String dcs) {
        ...
        mapFromObject.put(RsqlConstants.SYS_dataStatus, ReflectUtil.invokeMethod(RsqlConstants.SYS_Method_getDataStatus, o));
