...
public class DateHelper {
	...
	static public int getAge(final String dts) {
		...
		Date ld;int age =0;
		try {
			Assert.notNullAndEmpty(dts, ServiceCode.DATE_INVALID, "传参错误!不能为空或空值");
			ld = sdf.parse(dts);
	...
	static public Date parseDate(String dateString, String... format) {
		Assert.notNullAndEmpty(dateString, ServiceCode.DATE_INVALID, "日期字符串为空！");
	...
	static public Date parseDateTime(String dateString, String... format) {
		Assert.notNullAndEmpty(dateString, ServiceCode.DATE_INVALID, "日期时间字符串为空！");
		SimpleDateFormat sdf = new SimpleDateFormat(null == format || format.length < 1 ? "yyyy-MM-dd HH:mm:ss" : format[0]);
		try {
			return sdf.parse(dateString);
		} catch (ParseException e) {
			throw new DateException("格式化信息出错", e);
	...
	static public String formatDate(Date date, String... format) {
		Assert.notNullAndEmpty(date, ServiceCode.DATE_INVALID, "日期不能为空!");
		SimpleDateFormat sdf = new SimpleDateFormat(null == format || format.length < 1 ? "yyyy-MM-dd" : format[0]);
...
//		cal.set(cal.DAY_OF_YEAR, cal.get(cal.DAY_OF_YEAR)+diffDay);
	...
	public static int getDiffMonth(String startDateStr, String endDateStr){
		...
		Assert.isTrue((endDate.getTime() - startDate.getTime())>0, ServiceCode.FAIL, "起始日期须小于截止日期！");
