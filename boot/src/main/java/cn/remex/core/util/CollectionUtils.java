...
public abstract class CollectionUtils {
	...
	public static <T> Map<String, String> listToMap(List<T> list, String keyName, String valueName) {
		...
		Assert.notNull(list, "list不能为空" + keyName);
		if (list.size() == 0) {
			return map;
		}
		T o = list.get(0);

		Assert.notNull(ReflectUtil.getGetter(o.getClass(), keyName), "list转map时key必须为bean的属性" + keyName);
		Assert.notNull(ReflectUtil.getGetter(o.getClass(), valueName), "list转map时value必须为bean的属性" + valueName);
		for (T t : list) {
			/** 将指定的属性值作为key,对象值为o放入map @see ReflectUtil */
	 ...
	 * @return
	 */
	public static <T> String obtainColumnStr(List<T> list,String keyName){
		if(null == list || list.size()==0)
			return "";
		StringBuilder sb = new StringBuilder();
		for(T t:list){
			sb.append(ReflectUtil.invokeGetter(keyName, t)).append(",");
		}
		return sb.toString();
	}
