...
public class ReflectUtil implements RemexConstants {
	...
	final static private ReadOnlyMap<Class<?>, ReadOnlyMap<String, List<Method>>> AllMethods = new ReadOnlyMap<>();
	...
	final static private ReadOnlyMap<Type, Class<?>[]> ParameterClasses = new ReadOnlyMap<>();
...
//		return (Type[]) ReflectUtil.invokeMethod("getActualTypeArguments", type);
	...
	public static Class<?>[] obtainParameterClasses(final Type type) {
		try {
			...
			if(null!=curParameterClasses){return curParameterClasses;}//缓存中存在了
	...
	public static Map<String, Method> getSettersInContext(final Class<?> clazz,
		...
		final Map<String, Method> retSetters = new HashMap<>();
...
//	public static void obtainFieldName(Object o, Consumer aopBeanConsumer, Consumer<String> fieldNameConsumer) {
...
//			}
	...
	public static void copyProperties(final Object target, final Object source,
			...
			final SPFeature... features){
		Assert.notNull(target, ServiceCode.FAIL, "入参错误!目标对象为空!");
		Assert.notNull(source, ServiceCode.FAIL, "入参错误!源对象为空!");
	...
	public static void copyProperties(final Object target, final Object source,
		...
		Assert.notNull(target, ServiceCode.FAIL,"复制时目标对象不能为空");
		Assert.notNull(source, ServiceCode.FAIL,"复制时来源对象不能为空");
		Assert.isSimpleBean(target, ServiceCode.FAIL, "复制时目标对象必须是简单的Object对象，不能为Map或者Collection！");
		Assert.isSimpleBean(source, ServiceCode.FAIL,"复制时来源对象必须是简单的Object对象，不能为Map或者Collection！");
...
//						features);
	...
	public static void setProperties(final Object target, final Object source,
		...
		Assert.notNull(target, ServiceCode.FAIL, "通过Map对目标对象设置属性时，目标对象不能为空");
		Assert.notNull(source, ServiceCode.FAIL, "通过Map对目标对象设置属性时，来源对象不能为空");
		Assert.notNull(getValueMethod, ServiceCode.FAIL, "从来源对象获取值得方法不能为空");
