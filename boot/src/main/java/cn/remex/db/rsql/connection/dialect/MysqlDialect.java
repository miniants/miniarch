...
//		.append(") A WHERE ROWNUM<=").append(end).append(") WHERE rn>").append(start);
	...
	public String obtainSQLSelectTablesColumnNames(final String tableName) {
		...
		sqlString.append("select t.COLUMN_NAME,t.DATA_TYPE DATA_TYPE,t.CHARACTER_MAXIMUM_LENGTH DATA_LENGTH,t.COLUMN_COMMENT COMMENTS,t.NUMERIC_PRECISION,NUMERIC_SCALE from information_schema.columns t  where table_schema in (select database() ) and table_name= "+quoteAsString(tableName));
//
...
//				"and t.table_name = :tableName");
	...
	public String obtainSQLTypeString(final int type, final int length) {
		...
		default:
			throw new RsqlDialectException(ServiceCode.RSQL_DIALECT_ERROR, MysqlDialect.class+"中使用了未定义的SQLTypes!");
