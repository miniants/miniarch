...
import anbox.aibc.integral.SpreadIntegralSaver;
...
import cn.com.qqbx.cmsp.sms.xmlbean.ReqShortMessage;
...
import cn.remex.net.http.HttpHelper;
...
//}
	...
	public static String createPhoneCheckToken(final String phone) {
		String code = RandomUtils.createRandomCode(AiwbConsts.PhoneCheckCodeLen, false);// true表示包含字母
...
//		
		...
		phoneCheckCode.setValidityPeriod(AiwbConsts.PhoneCheckValidityPeriod);
		...
		cal.add(Calendar.MINUTE, AiwbConsts.PhoneCheckValidityPeriod);
		phoneCheckCode.setInvalidTime(DateHelper.formatDate(cal.getTime(), AiwbConsts.DateFormat_time));
	...
	public static boolean checkPhoneCheckToken(String phone, String code, String token) {
		...
		if (null == checkCode2 || DateHelper.parseDate(checkCode2.getInvalidTime(), AiwbConsts.DateFormat_time).getTime() < (new Date().getTime())) {
			if(null!=checkCode2){
