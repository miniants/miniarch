...
public class Node<T> {
    ...
    public <PT> Node(Node<PT> supNode, FieldType type, String nodeName) {
        ...
        if(TCollection.equals(type)) {
            ...
            if(null!=getter)
                this.nodeModel = (Class<T>) getter.getGenericReturnType();
        }
    }

    public <ST> Node<ST> addSubNode(FieldType type, String baseField, String nodeExpr) {
        Assert.isTrue(this.type.equals(FieldType.TCollection) || this.type.equals(FieldType.TObject), ServiceCode.FAIL, "只有Model节点或Collection节点能添加子节点！");
    ...
    private T obtainBean() {
        if (null == nodeBean) {
            if (TCollection.equals(type)) {
                ...
                Assert.notNull(id, ServiceCode.FAIL, "检索匹配的对象的id为空，无法赋值！");
