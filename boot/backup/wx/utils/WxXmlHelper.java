...
import zbh.remex.exception.JaxbException;
import zbh.remex.reflect.ReflectUtil;
import zbh.remex.util.Assert;
import zbh.remex.util.StringHelper;
...
public final class WxXmlHelper {
	 ...
	 * @return
	 */
	private static XStream getXStream(Class<?> c,final boolean CDATAFlag){
		XStream xStream = xStreamMap.get(c);
		if(null == xStream){
			xStream = new XStream(new DomDriver("UTF-8", new XmlFriendlyNameCoder("-_","_")){
...
//	}
	...
	public static String marshall(Object object,boolean CDATAFlag){
		return marshall(object,"UTF-8",CDATAFlag);
	}
	...
	public static String marshall(Object object,String sourceCharset,boolean CDATAFlag){
		if(object.getClass().isAnnotationPresent(XmlRootElement.class)){
			return useJaxbMarshall(object.getClass(), object, sourceCharset);
		}else{
			XStream xStream = getXStream(object.getClass(),CDATAFlag);
