...
public class RsqlRvo  extends DbRvo  {
	 ...
	 *                param is out of range (param &lt; 0 || param &gt;=
	 *                size()).
	...
	@Override
	public <T extends ModelableImpl> Object getCell(int rowNO, ColumnPredicate<T> cp, AggregateFunction aggregateFunction) {
		Param<String> fieldNameParam = new Param<>(null);
		ReflectUtil.eachFieldWhenGet(rsqlCvo.getBeanClass(), bean -> cp.init((T) bean), fieldName -> fieldNameParam.param = fieldName);
		return getCell(rowNO, Select.aliasAggregateFunction(aggregateFunction,fieldNameParam.param));
	}
	...
	@Override
	public <T extends ModelableImpl> List<Object> getCells(final ColumnPredicate<T> cp, final String columnValue, final ColumnPredicate<T> needColumnNameCp, AggregateFunction aggregateFunction) {
		Param<String> columnName = new Param<>(null);
		ReflectUtil.eachFieldWhenGet(rsqlCvo.getBeanClass(), bean -> cp.init((T) bean), fieldName -> columnName.param = fieldName);

		Param<String> needColumnName = new Param<>(null);
		ReflectUtil.eachFieldWhenGet(rsqlCvo.getBeanClass(), bean -> cp.init((T) bean), fieldName -> needColumnName.param = fieldName);

		return getCells(getTitleIndex(columnName.param), columnValue, getTitleIndex(Select.aliasAggregateFunction(aggregateFunction,needColumnName.param)));
	}
	@Override
	public <T extends ModelableImpl> Object getCell(final ColumnPredicate<T> columnCp, final String columnValue, final ColumnPredicate<T> needColumnNameCp, AggregateFunction aggregateFunction) {
		Param<String> columnName = new Param<>(null);
		ReflectUtil.eachFieldWhenGet(rsqlCvo.getBeanClass(), bean -> columnCp.init((T) bean), fieldName -> columnName.param = fieldName);

		Param<String> needColumnName = new Param<>(null);
		ReflectUtil.eachFieldWhenGet(rsqlCvo.getBeanClass(), bean -> needColumnNameCp.init((T) bean), fieldName -> needColumnName.param = fieldName);

		return getCells(getTitleIndex(columnName.param), columnValue, getTitleIndex(Select.aliasAggregateFunction(aggregateFunction,needColumnName.param))).get(0);
	}

	/**
	 * 获取当前页数据中的某一列的数据
	 * @rmx.summary 数据类型默认为String，如果需要转化，可用同名函数getColumn(int param,Class clazz)
	 * @param index
	...
	@Override
	public <T extends ModelableImpl>  TreeMap<String, String> getMapFromColumns(ColumnPredicate<T> keyCp, ColumnPredicate<T> valueCp, AggregateFunction valueAggregateFunction) {
		Param<String> keyParam = new Param<>(null);
		Param<String> valueParam = new Param<>(null);
		ReflectUtil.eachFieldWhenGet(rsqlCvo.getBeanClass(), bean -> keyCp.init((T) bean), fieldName -> keyParam.param = fieldName);
		ReflectUtil.eachFieldWhenGet(rsqlCvo.getBeanClass(), bean -> valueCp.init((T) bean), fieldName -> valueParam.param = fieldName);
		return getMapFromColumns(keyParam.param,Select.aliasAggregateFunction(valueAggregateFunction,valueParam.param));
	}
	/**
...
//		return objects;
	...
	@Override
	public <T> Map<String, T> obtainObjectsMap(final String columnName) {
		List<T> beans = this.obtainObjects((Class<T>) this.beanClass);
...
//			System.out.println("---------------"+rows.size());
	...
	private <T> List<T> obtainBeans(Class<T> clazz,boolean returnModel,T targetBean) {
		...
		rsqlCvo.getContainer().setLocalAutoStoreObjectFlied(false);// 此处是数据库操作，禁用自动保存的标志
		rsqlCvo.getContainer().setLocalAutoFecthObjectFiled(false);// 此处是数据库操作，禁用自动读取的标志
		ArrayList<T> list = new ArrayList<T>();
		// 逐行放入数据
		int rowCount = this.rows.size(), columnCount = this.titles.size();
		//int idIdx = this.titles.indexOf("id");
		Container spaceConfig =  rsqlCvo.getContainer();
		...
		for (int i = 0; i < rowCount; i++) {
			...
			if(null!=targetBean){
				...
				if((needAdd = null==bean)){
					if(_returnModel){
						@SuppressWarnings("unchecked")
						T bean1 = (T)rsqlCvo.getContainer().createDBBean(clazz);
						bean = bean1;
					}else{
			...
			if(returnModel)
				rootNode.start(bean, this, i, rsqlCvo.getContainer(), method);
			else
			...
			for (int j = 0; j < columnCount; j++) {
				...
				//setValue(this.rsqlCvo._getSpaceName(), clazz, t, bean, value, i, status);
		...
		// 删除多余空间
		list.trimToSize();
		rsqlCvo.getContainer().setLocalAutoStoreObjectFlied(true);
		rsqlCvo.getContainer().setLocalAutoFecthObjectFiled(true);
		return list;
	}
	...
	private void setValue(final String poolName, final Class<?> clazz,
		...
		if (null != setter) {
			...
			if (null == objectValue && !"-1".equals(pk)) {
				...
				objectValue = rsqlCvo.getContainer().createDBBean(fieldType);
