...
public final class RsqlUtils implements RsqlConstants {
	...
	static {
		...
		SqlTypes.put(long.class, new ColumnType(Types.NUMERIC, 22));
	 ...
	 */
	public static DbRvo doManyToMany_delete(String spaceName, Class<?> beanClass, final String beanName,
	...
	public static DbRvo doManyToMany_insert(
			String spaceName, final Class<?> beanClass, final String beanName,
...
//		Type[] types = typeImpl.getActualTypeArguments();
	...
	public static <T extends Modelable> String obtainSQLOrder(final DbCvo<T> cvo) {
		...
		if(!cvo._isHasGroupBy())
			result.append(dialect.quoteFullName(cvo._getTableAliasName(),SYS_id)).append(" DESC, ");
	...
	public static ColumnType obtainSQLType(final Type type, final String fieldName, final Class<?> beanClass) {
		...
		// 如果是通过RemexColumn声明
		if (null != beanClass  && null !=(sqlType=SqlTypes.get(type)) && null!=(at = ReflectUtil.getAnnotation(beanClass, fieldName, Column.class))) {
		...
		if (null == sqlType && SqlType.isTObject(type)) {
			...
			return  null==at?new ColumnType(Types.CHAR,100):new ColumnType(Types.CHAR,at);
	 ...
	 * @return DbRvo
	 */
	public static DbRvo queryCollectionBeans(String spaceName, final Class<?> beanClass, final String fieldName, final Object beanId) {
		Dialect dialect = RDBManager.getLocalSpaceConfig(spaceName).getDialect();
		...
		if(isOnetoMany)Assert.isTrue(null!=oneToMany.mappedBy(), ServiceCode.FAIL, "一对多必须制定多方维护的属性字段",RsqlConnectionException.class);
		...
		if (!isOnetoMany) {
			...
			if (null != manyToMany) {// 显示声明多对多，双方共同维护。
				...
				Assert.notNull(mb, ServiceCode.FAIL,  new StringBuilder("显式指定为双方维护的多对多映射中本类").append(beanClass.getName()).append("没有指定对方类多对多属性:").append(mb).toString(), RsqlConnectionException.class);
				Assert.isTrue(tcGetters.containsKey(mb), ServiceCode.FAIL, new StringBuilder(" 显式指定为双方维护的多对多映射中对方类").append(subBeanClass.getName()).append("不存在该多对多属性:").append(mb).toString(), RsqlConnectionException.class);

				// 获取对方类多对多声明
				ManyToMany tMtm = ReflectUtil.getAnnotation(subBeanClass, mb, ManyToMany.class);
				Assert.notNull(tMtm, ServiceCode.FAIL,  "显示声明多对多时，需要双方指定ManyToMany声明，此处对方类的ManyToMany声明为空！请更正！", RsqlConnectionException.class);
				tmb = tMtm.mappedBy();// 对方类的多对多属性

				Assert.notNull(tmb, ServiceCode.FAIL,  new StringBuilder("显式指定为双方维护的多对多映射中对方类").append(subBeanClass.getName()).append("没有指定本类的多对多属性:").append(mb).toString(), RsqlConnectionException.class);

				Class<?> te = manyToMany.targetEntity(); // 检查主从关系 本类
				Class<?> tte = tMtm.targetEntity(); // 对方类

				Assert.isTrue((null == te && null == tte) || (null != te && null != tte), ServiceCode.FAIL,  new StringBuilder("显式指定为双方维护的多对多映射中，ManyToMany声明不能同时设置或者同时为空！设置targetEntity的为主维护方。").append(mb).toString(),
	 ...
	 * @return SqlBean
	 */
	public static <T extends Modelable> void createDeleteSql(final DbCvo<T> cvo) {
		Dialect dialect = RDBManager.getLocalSpaceConfig(cvo._getSpaceName()).getDialect();

		ArrayList<NamedParam> namedParams = new ArrayList<NamedParam>();

		StringBuilder sqlString = new StringBuilder();
		sqlString.append("DELETE FROM ").append(dialect.openQuote()).append(cvo.getBeanName()).append(dialect.closeQuote()).append(" WHERE ").append(dialect.quoteKey(SYS_id)).append("= :").append(SYS_id);
		// 默认索引都是整型
	 ...
	 * @return SqlBean
	 */
	public static <T extends Modelable> void createInsertSqlBean(final DbCvo<T> cvo) {
		Dialect dialect = RDBManager.getLocalSpaceConfig(cvo._getSpaceName()).getDialect();
...
//		sqlBean.setTableName(beanName);
		...
		for (String column : SysColumns.keySet()) {
			...
			namedParams.add(new NamedParam(-1, column, SysColumns.get(column).type, null));
		}
		...
		for (String column : baseCols.keySet()) {
			part_column_sb.append(dialect.quoteKey(column)).append(",");
			part_value_sb.append(":").append(column).append(",");
			namedParams.add(new NamedParam(-1, column, baseCols.get(column).type, null));
		}
		...
		if (dataOption.contains(DT_object)) {
			...
			for (String column : objectCols.keySet()) {
				part_column_sb.append(dialect.quoteKey(column)).append(",");
				part_value_sb.append(":").append(column).append(",");
				namedParams.add(new NamedParam(-1, column, objectCols.get(column).type, null));
			}
		}
	 ...
	 * @throws Exception
	 */
	public static <T extends Modelable> void createSelectSqlBean(final DbCvo<T> cvo) {
		String dtCols = cvo.getDataColumns();
		...
		if(!cvo._isHasGroupBy() &&!cvo._isHasAggregateBy()){
...
//			select.appendBaseColumn(SYS_name, SYS_name);
		...
		if(hasNotBaseColumn.param && ! cvo._isSubStatment() && !cvo._isHasGroupBy() && !cvo._isHasAggregateBy()){
			Class<?> beanClass=cvo.getBeanClass();
...
//	}
	...
	private static StringBuilder obtainDecodeSqlPart(
		...
		DbCvo dbCvo = ContainerFactory.createDbCvo(clazz);
		...
		Assert.isTrue(map!=null && map.size()>0, ServiceCode.FAIL,  "配置的值参照中没有相应的参数！请配置人员查阅数据表:"+codeTableName);
	...
	private static StringBuilder obtainRsqlDecodePart(String spaceName,
		...
		Assert.isTrue(map!=null && map.size()>0, ServiceCode.FAIL,  "配置的值参照中没有相应的参数！请配置人员查阅数据表:"+DataDicTb);
...
//		sb.append(") ").append(dialect.quoteKey("RMX_"+DataDicTb+"_"+fieldName));
	 ...
	 * @return SqlBean
	 */
	public static <T extends Modelable> void createSqlStringSqlBean(final DbCvo<T> cvo) {
		Dialect dialect = RDBManager.getLocalSpaceConfig(cvo._getSpaceName()).getDialect();
...
//		sqlBean.setTableName(_tableAliasName==null || "WHERE".equals(_tableAliasName.toUpperCase())?tableName:_tableAliasName);
	 ...
	 * @return SqlBean
	 */
	public static <T extends Modelable> void createUpdateSqlBean(final DbCvo<T> cvo) {
		Dialect dialect = RDBManager.getLocalSpaceConfig(cvo._getSpaceName()).getDialect();
...
//		sqlBean.setTableName(beanName);
		...
		String part_where = Judgment.nullOrBlank(cvo.getId())?
				"\r\n  " + cvo.getFilter().toSQL(true, tableAliasName, namedParams, paramIndex,tableIndex, null)
		...
		for (String column : SysModifyColumns.keySet()) {
			...
			namedParams.add(new NamedParam(-1, column, SysModifyColumns.get(column).type, null));
		}
		...
		if (dataOption.contains(DT_object)) {
			...
			for (String column : objectCols.keySet()) {
				...
				namedParams.add(new NamedParam(-1, column, Types.CHAR, null));// 所有对象引用都是整型
			}
		}
